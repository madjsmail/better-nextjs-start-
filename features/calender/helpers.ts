...
import { TCalendarView, TVisibleHours, TWorkingHours } from "./types";
...
export function isWorking(day: Date, dayFrom: Date, dayTo: Date) {
  ...
  const df = new Date(dayFrom.getFullYear(), dayFrom.getMonth(), dayFrom.getDate());
  const dt = new Date(dayTo.getFullYear(), dayTo.getMonth(), dayTo.getDate());
